    ...
    <main className="min-h-screen bg-[#fbf7ee] text-[#151515]">
      ...
      <article className="grid gap-6 px-4 py-6 md:grid-cols-[minmax(0,1.2fr)_minmax(320px,0.8fr)] md:px-6">
        <div className="border border-black/10 bg-white">
          ...
          <img src={imageUrl} alt={sign.sign_title} className="max-h-[78vh] w-full object-contain p-4" />
        </div>

        <section className="grid content-start gap-5">
          <div>
            <h1 className="text-3xl font-semibold tracking-normal">{sign.sign_title}</h1>
            <p className="mt-2 text-lg text-black/70">{sign.restaurant_name}</p>
          </div>
          ...
          <dl className="grid gap-px overflow-hidden border border-black/10 text-sm">
            {[
              ...
              ["Date collected", sign.date_collected],
            ].map(([label, value]) => (
